...
main_directory = uigetdir(fullfile(pwd, 'Library Data/Si_on_SiO2_square_with_cap'), ...
...
% Regex to extract the numeric angle from filenames (e.g., theta0.mat, theta10.mat, theta-10.mat)
theta_pat = 'theta(-?\d+)\.mat$';

total_theta_files = 0;

%% Walk the tree and ingest every theta*.mat (Square pillars: width sweep)
for h_idx = 1:numel(height_folders)
    ...
    for p_idx = 1:numel(pitch_folders)
        ...
        for tf_idx = 1:numel(theta_files)
            ...
            theta_val_str = tok{1};                   % e.g., '0', '10', '-10'
            theta_key = ['theta' theta_val_str];      % canonical key, e.g., 'theta0', 'theta10', 'theta-10'
            ...
            % data_values{1}    = complex field -> use |.|^2 for intensity
            ...
            try
                ...
                % Intensity (|t|^2)
                trans_data = filedata.data_values{1} .* filedata.data_values{1};
            ...
            % Build rows (one row per width) and names for this file
            row_block  = zeros(no_of_width, numel(wavelength_values));
            name_block = cell(1, no_of_width);

            for w_idx = 1:no_of_width
                % Expecting trans_data sized [no_of_wavelength x no_of_width x ...]
                row_block(w_idx, :) = (squeeze(trans_data(:, w_idx, 1))).';
                name_block{w_idx} = sprintf('%s_%s_Width_%.3f_Si_SquarePillars', ...
                    height_folders(h_idx).name, pitch_folders(p_idx).name, width_values(w_idx));
            end

            % Append into theta_map (enforcing wavelength consistency per theta)
...
for k = 1:numel(theta_keys)
    ...
    % File names
    csv_name = fullfile(main_directory, sprintf('TransmissionTable_Square_NIR_%s.csv', key));
    ...
    title(sprintf('All Square Pillar Transmission Spectra — %s', key), 'FontSize', 14);
    grid on; ylim([0 1]); box on; hold off;

    png_name = fullfile(main_directory, sprintf('AllTransmissionSpectra_SquarePillars_%s.png', key));
    fig_name = fullfile(main_directory, sprintf('AllTransmissionSpectra_SquarePillars_%s.fig', key));
    try
        saveas(f, png_name);
        saveas(f, fig_name);
