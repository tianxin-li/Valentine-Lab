...
main_directory = uigetdir(fullfile(pwd, 'Library Data/Si_on_SiO2_cross_with_cap/'), ...
...
%% Walk the tree and ingest every theta*.mat (Cross pillars: width × aspect)
for h_idx = 1:numel(height_folders)
    ...
    for p_idx = 1:numel(pitch_folders)
        ...
        for tf_idx = 1:numel(theta_files)
            ...
            % data_values{3}    = transmission for crosses, size [Nwavelength x Nwidth x Naspect (x ...)]
            ...
            try
                ...
                % Transmission for crosses (use as-is; if your values are complex fields, apply abs(.)^2 here)
            ...
            for w_idx = 1:no_of_width
                for a_idx = 1:no_of_aspect
                    ...
                    name_block{r} = sprintf('%s_%s_Width_%.3f_AspectRatio_%.2f_Cross', ...
                        height_folders(h_idx).name, pitch_folders(p_idx).name, ...
                        width_values(w_idx), aspect_values(a_idx));
                end
            end

            % Append into theta_map (keep wavelength grid consistent within a theta)
...
for k = 1:numel(theta_keys)
    ...
    % Save CSV
    csv_name = fullfile(main_directory, sprintf('TransmissionTable_Cross_%s.csv', key));
    ...
    title(sprintf('All Cross Transmission Spectra — %s', key), 'FontSize', 14);
    grid on; ylim([0 1]); box on; hold off;

    png_name = fullfile(main_directory, sprintf('AllTransmissionSpectra_Cross_%s.png', key));
    fig_name = fullfile(main_directory, sprintf('AllTransmissionSpectra_Cross_%s.fig', key));
    try
        saveas(f, png_name);
        saveas(f, fig_name);
